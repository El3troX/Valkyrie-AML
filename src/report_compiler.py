...
def generate_sar_narrative(
    ...
    system_inst = (
        ...
        "CRITICAL RULE: Do NOT write long paragraphs. Write the entire report using clear, detailed, numbered pointers or bullet points (using '-' or '•') for every section and description. "
        "Do NOT output any bracketed placeholder text like [Date] or [Account Holder Name]. "
...
def _template_narrative(
    ...
    return (
        ...
        f"• An investigation was conducted on Subject Account {account_id} following automated flagging by Valkyrie-AML.\n"
        f"• During the observation window, the account conducted {txns:,} transactions with a cumulative volume of ${amount:,.2f}.\n"
        f"• The peak anomaly score reached {score:.3f}, meeting the threshold for mandatory SAR documentation.\n\n"
        f"2. TYPOLOGY & RED FLAG ANALYSIS\n"
        f"• Quantitative feature analysis and SHAP explainability models identified primary compliance red flags:\n"
        f"{red_flags_text}\n"
        f"• These indicators are strongly characteristic of rapid layering, smurfing, and structuring designed to evade reporting thresholds.\n\n"
        f"3. NETWORK RISK & COUNTERPARTY EXPOSURE\n"
        f"• Graph analytics and Personalised PageRank (PPR) risk propagation identified key network connections between Account {account_id} and counterparty accounts: {conn_str}.\n"
        f"• Risk scores propagated through the network indicate potential coordinated multi-account laundering.\n\n"
